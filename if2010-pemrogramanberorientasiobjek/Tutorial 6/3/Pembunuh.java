...
public class Pembunuh implements Kelas {
    ...
    public void serang(Karakter self, Karakter target) {
        String s = self.getName() + ", sang pembunuh, menyerang " + target.getName() + " dengan " + this.senjata;
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()) {
            ...
            int takenHealth = (targetHealth >= 20) ? 20 : targetHealth;

            System.out.println(self.getName() + " menyerap health " + target.getName() + " sebesar " + takenHealth);
            
            target.terimaSerangan(takenHealth);
            self.addHealth(takenHealth);
        } else {
