...
IdxType getLastIdxRow(Matrix m) { return ROW_EFF(m) - 1; }

IdxType getLastIdxCol(Matrix m) { return COL_EFF(m) - 1; }

boolean isIdxEff(Matrix m, IdxType i, IdxType j) {
    return i >= 0 && i < ROW_EFF(m) && j >= 0 && j < COL_EFF(m);
}

ElType getElmtDiagonal(Matrix m, IdxType i) { return ELMT(m, i, i); }
...
void displayMatrix(Matrix m) {
    for (int i = 0; i < ROW_EFF(m);
         ++i) {  // Ubah batas loop menjadi < ROW_EFF(m)
        for (int j = 0; j < COL_EFF(m);
             ++j) {  // Ubah batas loop menjadi < COL_EFF(m)
            if (j > 0) {
                printf(" ");  // Tambahkan spasi sebelum elemen jika bukan
                              // elemen pertama
            }
...
int countElmt(Matrix m) { return ROW_EFF(m) * COL_EFF(m); }

boolean isSquare(Matrix m) { return ROW_EFF(m) == COL_EFF(m); }
...
boolean isSparse(Matrix m) {
    int totalElements =
        ROW_EFF(m) * COL_EFF(m);  // Total elemen efektif dalam matriks
    ...
    // Jika jumlah elemen non-nol kurang dari atau sama dengan 5% dari total
    // elemen
    return (nonZeroCount <= PERCENTAGE_SPARSE * totalElements);
}
//====================================================================================================

Matrix negation(Matrix m) { return multiplyByConst(m, -1); }

void pNegation(Matrix *m) { pMultiplyByConst(m, -1); }
