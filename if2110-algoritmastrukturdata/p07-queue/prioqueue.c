...
void switchBefore(PrioQueue* pq, int idx) { // selalu switch dengan idx-1 (sebelumnya)
    if (idx == 0) {
...
void dequeue(PrioQueue *pq, ElType *val) {
    *val = HEAD(*pq);
    ...
    if (IDX_HEAD(*pq) == IDX_TAIL(*pq)) { //kasus hanya satu elemen sehingga menjadi kosong
        IDX_HEAD(*pq) = IDX_UNDEF;
        IDX_TAIL(*pq) = IDX_UNDEF;
        return;
    }
